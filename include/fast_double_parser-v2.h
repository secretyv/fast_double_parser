#ifndef FAST_DOUBLE_PARSER_V2_H
#define FAST_DOUBLE_PARSER_V2_H
...
namespace fast_double_parser_v2 {
...
static const int FASTFLOAT_SMALLEST_POWER = -325;  //std::numeric_limits<double>::min_exponent10;
static const int FASTFLOAT_LARGEST_POWER  =  308;  //std::numeric_limits<double>::max_exponent10;
static const int MAX_DIGITS = 19;                  //std::numeric_limits<uint64_t>::digits10;
static const int MAX_DGTEXP = MAX_DIGITS - 1;
...
static uint64_t Emulate64x64to128(uint64_t& r_hi, const uint64_t x, const uint64_t y) {
...
static bool parse_float_strtod(int64_t ie, uint64_t i, bool negative, double* outDouble) {
   char str[32] = "";
   char *first, *endptr;

   first = str;
   itoa_branchlut2::itoa(i, first);
   first = str + strlen(str); 
   *first++ = '.';
   *first++ = 'e';
   itoa_branchlut2::itoa(ie, first);
#if defined(__CYGWIN__) || defined(__MINGW32__) || defined(__MINGW64__) 
   // workround for cygwin
   *outDouble = cygwin_strtod_l(str, &endptr);
#elif defined(_WIN32)
   static _locale_t c_locale = _create_locale(LC_ALL, "C");
   *outDouble = _strtod_l(str, &endptr, c_locale);
#else
   static locale_t c_locale = newlocale(LC_ALL_MASK, "C", NULL);
   *outDouble = strtod_l(str, &endptr, c_locale);
#endif
   ...
   if ((endptr == str) || (!std::isfinite(*outDouble))) {
      return false;
   }
   if (negative) *outDouble = -*outDouble;
   return true;
}

// parse the number at first
// return NULL in case if error
// else return the new position in the input string
template<typename InputIt, typename OutputType>
WARN_UNUSED
really_inline InputIt parse_number(InputIt first, InputIt last, OutputType& outDouble) {
   ...
   if (*first == '-') {
      ++first;
      if (first == last) throw std::invalid_argument("Invalid record");
      negative = true;
   }
   else if (*first == '+') {
      ++first;
   ...
   // Get integral part
   uint64_t mantissa = 0;              // mantissa - an unsigned int avoids signed overflows (which are bad)
   int64_t  exponent = 0;
   char     dgt_cdwn = MAX_DIGITS;     // digit countdown
   if (*first != '.') {
      if (*first == '0') {          // 0 cannot be followed by an integer
         ++first;
         if (is_integer(*first) && first != last) 
         ...
         --dgt_cdwn;
         ++first;
         while (is_integer(*first) && dgt_cdwn > 0 && first != last) {
            mantissa = 10 * mantissa + (*first - '0');
            --dgt_cdwn;
            ++first;
         }
         if (unlikely(dgt_cdwn <= 0)) {
            while (is_integer(*first) && first != last) {
               ++exponent;
               ++first;
            }
         }
   ...
   if (*first == '.') {
      ++first;
      // if (!is_integer(*first) && first != last)    // must start with an integer
      //    throw std::invalid_argument("Invalid record");
   ...
   if (('e' == *first) || ('E' == *first)) {
      ...
      int64_t exp_numbr = (*first - '0');
      first++;
      if (is_integer(*first) && first != last) {
         exp_numbr = 10 * exp_numbr + (*first - '0');
         ++first;
      }
      if (is_integer(*first) && first != last) {
         exp_numbr = 10 * exp_numbr + (*first - '0');
      ...
      while (is_integer(*first) && first != last) {
         if (exp_numbr < 0x100000000) { // we need to check for overflows
            exp_numbr = 10 * exp_numbr + (*first - '0');
         }
         ++first;
      }
   ...
   if (exponent >= FASTFLOAT_SMALLEST_POWER && exponent <= FASTFLOAT_LARGEST_POWER) {
      // from this point forward, exponent >= FASTFLOAT_SMALLEST_POWER and
      // exponent <= FASTFLOAT_LARGEST_POWER
      bool success = true;
      outDouble = compute_float_64(exponent, mantissa, negative, &success);
      if (!success) {
         // we are almost never going to get here.
         return parse_float_strtod(exponent, mantissa, negative, &outDouble) ? first : NULL;
      }
   }
   else {
...
// else return the new position in the input string
WARN_UNUSED
really_inline const char* parse_number(const char* first, double& outDouble) {
   ...
   if (*first != '.') {
      if (*first == '0') {                // 0 cannot be followed by an integer
         ++first;
         if (is_integer(*first)) return NULL;
      }
      else if (is_integer(*first)) {
         mantissa = (*first - '0');
         --dgt_cdwn;
         ++first;
         while (is_integer(*first) && dgt_cdwn > 0) {
            mantissa = 10 * mantissa + (*first - '0');
            --dgt_cdwn;
            ++first;
         }
   ...
   if (*first == '.') {
      ++first;
      // if (!is_integer(*first))         // must start with an integer
      //    return NULL;
   ...
   if (('e' == *first) || ('E' == *first)) {
      ...
      exponent += (neg_exp ? -exp_numbr : exp_numbr);
   }
...
/*
Work template function to parse many values.
...
really_inline InputIt parse_many(InputIt first, InputIt last, OutputIt result, size_t maxItems)
{
   try
   {
      OutputType val;
      ...
      {
         first = private_part::parse_number(first, last, val);
         *result = val;
         ++result;
         ++n;
      }
...
/*
Work template function to parse one value.
Read from first to at most last, extracting one number to result.
Returns the updated iterator or NULL on error.
*/
template <typename InputIt, typename OutputIt, typename OutputType>
really_inline InputIt parse_one(InputIt first, InputIt last, OutputIt result)
{
   try
   {
      OutputType val;
...
really_inline char const* parse_number(char const* first, double* result)
{
   return private_part::parse_number(first, *result);
}

// Parse one number at first in result.
// On success return the new position, on error return NULL.
template <typename Container>
really_inline char const* parse_number(char const* first, std::back_insert_iterator<Container> result)
{
...
really_inline char const* parse_number(char const* first, OutputIt result)
{
   typedef char const* InputIt;
...
really_inline char const* parse_number(char const* first, char const* last, ResType* result)
{
   typedef char const* InputIt;
...
really_inline InputIt parse_numbers(InputIt first, 
                                    InputIt last, 
                                    ResType* result, 
